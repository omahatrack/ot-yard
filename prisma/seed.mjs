...
function hashPassword(password){ const salt=crypto.randomBytes(16).toString("hex"); const hash=crypto.pbkdf2Sync(password,salt,120000,32,"sha256").toString("hex"); return `${salt}:${hash}`; }
...
async function main() {
  ...
  for (const u of userSeeds) {
    ...
    if (!user) user = await prisma.user.create({data:{name:u.name,username:u.username,passwordHash:hashPassword(u.password),roleId:u.roleId,locationId:location.id}});
    await sql('INSERT INTO UserAccessSettings(userId,isActive) VALUES (?,TRUE) ON DUPLICATE KEY UPDATE userId=VALUES(userId)',[user.id]);
    await sql('INSERT INTO UserLocationAccess(userId,locationId,roleId,isActive) VALUES (?,?,?,TRUE) ON DUPLICATE KEY UPDATE userId=VALUES(userId)',[user.id,location.id,u.roleId]);
  }
  ...
  const equipmentIdByCode = new Map();
